...
> implemented in `pipeline/circuit_spec.py`; this is the research basis.
...
**Score (single source of truth: `pipeline/evaluate.py::evaluate()` — the E2E evaluator; the retired `circuit_spec.score()` predates the single-evaluator redesign)**:
