 ...
 *
 * This estimator is RULED OUT at FID SNRs (crossing timestamps destroy
 * the inter-sample phase continuity where the information lives). It
 * exists in this core so the ruling-out is an E2E result scored through
 * the identical path as the zoom core (REDESIGN.md section 2) -- the
 * regression lives in tests/test_estimator_reference.py and the E2E gate
 * failure is locked by tests/test_circuit_spec_regression.py.
 */
#include "freq_est.h"
