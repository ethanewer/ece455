"""NumPy mirror of the portable C frequency estimator.
...
This algorithm mirror exists only for C-side debugging. A C-versus-mirror
difference localizes numeric or porting bugs such as NCO table, FIR indexing,
or fixed-point saturation errors. Production code uses the C implementation.
"""
import numpy as np
...
def fe_mirror(v: np.ndarray, fs: float, t0: float, f_lo: float, f_hi: float
    ...
    for kk in np.array_split(k, max(1, len(k) // 128)):
        ...
        # Avoid platform BLAS complex-matmul warning bugs on finite inputs.
        dft = np.sum(np.exp(-1j * a) * x[:nseed], axis=1)
        sp = np.abs(dft) ** 2
        j = int(np.argmax(sp))
        if sp[j] > best:
            best = sp[j]
