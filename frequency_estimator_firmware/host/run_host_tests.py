...
committed golden vectors (`frequency_estimator_firmware/host/vectors/`)
and cross-validates against the numpy mirror of the C algorithm
(`frequency_estimator_firmware/host/freq_est_mirror.py`):
...
Run:  make -C frequency_estimator_firmware/core test
"""
import ctypes
import hashlib
import json
...
VEC = ROOT / "frequency_estimator_firmware" / "host" / "vectors"
sys.path.insert(0, str(ROOT / "frequency_estimator_firmware" / "host"))
...
def main() -> int:
    ...
    spec = importlib.util.spec_from_file_location(
        "freq_est_mirror", str(ROOT / "frequency_estimator_firmware" / "host" / "freq_est_mirror.py"))
