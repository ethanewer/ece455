...
derivatives, including cross-terms between amplitude, frequency, decay, and
phase, so dead time, finite records, and parameter coupling are handled. The
frequency CRB is [J^-1]_ff. `tests/test_crb.py` checks the white-noise result
against the real-tone closed form and checks the shaped-noise implementation
against the flat band-limited case.
...
def freq_crb_colored(t, amp, freq, tau, phase, fs, f_lo, f_hi, sigma):
    ...
    `tests/test_crb.py` checks the shaped-noise form against this flat-band
    special case.
    """
    p = partials(t, amp, freq, tau, phase)
    n = len(t)
...
def crb_nt(t, amp, freq, tau, phase, sigma):
    ...
    return freq_crb(t, amp, freq, tau, phase, sigma) / physics.GAMMA_HZ_PER_NT
