...
def build_spice_report(
    ...
    *,
    ...
    if not 0 <= transient_start_s < transient_stop_s:
        raise ValueError("transient_start_s must precede transient_stop_s")
    ...
    gain_axis.axvline(marker_hz, color="#555", linestyle="--", linewidth=1)
    ...
    index = int(np.argmin(np.abs(frequency_hz - marker_hz)))
    settled = time_s >= (transient_start_s + transient_stop_s) / 2
    ...
    summary_md.write_text(
        ...
        f"- Nominal frequency marker: {marker_hz:.3f} Hz\n"
